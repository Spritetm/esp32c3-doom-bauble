                ...
                void *arg)
{
    ...
    case 0:
		...
		printf("Discovered char handle %"PRIu16" desc 0x%"PRIx16"\n", dsc->handle, dsc->uuid.u16.value);
        break;
                ...
                const struct ble_gatt_chr *chr, void *arg)
{
    ...
    switch (error->status) {
    case 0:
		printf("Discovered char handle %"PRIu16" UUID 0x%"PRIx16"\n", chr->def_handle, chr->uuid.u16.value);
...
peer_chr_find_uuid_nth(const struct peer *peer, const ble_uuid_t *svc_uuid,
                   const ble_uuid_t *chr_uuid, int nth)
{
    const struct peer_svc *svc;
    ...
    if (svc == NULL) {
		printf("peer_chr_find_uuid: did not find svc_uuid\n");
        return NULL;
    }

    SLIST_FOREACH(chr, &svc->chrs, next) {
//		printf("peer_chr_find_uuid: Checking %04X\n", ble_uuid_u16(&chr->chr.uuid.u));
...
peer_chr_find_uuid(const struct peer *peer, const ble_uuid_t *svc_uuid,
	...
	return peer_chr_find_uuid_nth(peer, svc_uuid, chr_uuid, 0);
}

const struct peer_dsc *
peer_dsc_find_uuid_nth(const struct peer *peer, const ble_uuid_t *svc_uuid,
                   const ble_uuid_t *chr_uuid, const ble_uuid_t *dsc_uuid, int nth)
{
    const struct peer_chr *chr;
    const struct peer_dsc *dsc;

    chr = peer_chr_find_uuid_nth(peer, svc_uuid, chr_uuid, nth);
    if (chr == NULL) {
        return NULL;
    }

    SLIST_FOREACH(dsc, &chr->dscs, next) {
//		printf("peer_dsc_find_uuid: Checking dsc %04X\n", ble_uuid_u16(&dsc->dsc.uuid.u));
...
peer_dsc_find_uuid(const struct peer *peer, const ble_uuid_t *svc_uuid,
	...
	return peer_dsc_find_uuid_nth(peer, svc_uuid, chr_uuid, dsc_uuid, 0);
}
                ...
                const struct ble_gatt_svc *service, void *arg)
{
    ...
    switch (error->status) {
    case 0:
		printf("Discovered service handle %"PRIu16" desc 0x%"PRIx16"\n", service->start_handle, service->uuid.u16.value);
